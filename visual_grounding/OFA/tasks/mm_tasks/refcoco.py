    ...
    def _inference(self, generator, sample, model):
        ...
        for i in range(len(gen_out)):
            try: 
                ...
                #print("!!!!!gen_out!!!!!!",gen_out[i][0]["tokens"][:-1] - len(self.src_dict) + self.cfg.num_bins)
                refs.append(sample["target"][i][:-1] - len(self.src_dict) + self.cfg.num_bins)
            except:
                print("gen_out[{}] = {}".format(i,gen_out[i]))
                #exit()
                hyps.append(sample["target"][i][:-1] - len(self.src_dict) + self.cfg.num_bins)
                refs.append(sample["target"][i][:-1] - len(self.src_dict) + self.cfg.num_bins)
