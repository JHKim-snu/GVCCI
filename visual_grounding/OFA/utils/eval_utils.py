...
def eval_caption(task, generator, models, sample, **kwargs):
    ...
    score = None
    if 'score' in hypos[0][0].keys():
        score = hypos[0][0]['score']
...
# def eval_progrounding(task, generator, models, sample, **kwargs):
#     gen_out = task.inference_step(generator, models, sample)
#     hyps = []
#     for i in range(len(gen_out)):
#         hyps.append(gen_out[i][0]["tokens"][:-1] - len(task.src_dict) + task.cfg.num_bins)
...
#         _hyps = []
#         for slice_index in range(len(hyp) // 4):
#             _hyp = hyp[0 + (slice_index * 4) : 4 + (slice_index * 4)]
#             _hyp = _hyp / (task.cfg.num_bins - 1) * task.cfg.max_image_size
#             _hyp[::2] /= sample['w_resize_ratios'][0]
#             _hyp[1::2] /= sample['h_resize_ratios'][0]
#             _hyps.append(list(_hyp.cpu().numpy()))
#         hyp_array.append(list(_hyps))
#     results = []
#     for i, sample_id in enumerate(sample["id"].tolist()):
#         sample = {}
#         sample["unique_id"] = sample_id
#         sample["box"] = hyp_array[i]
...
def eval_step(task, generator, models, sample, **kwargs):
    ...
    elif task.cfg._name == "gc":
        return eval_caption(task, generator, models, sample, **kwargs)
    elif task.cfg._name == "questioner":
        return eval_caption(task, generator, models, sample, **kwargs)
    elif task.cfg._name == "answerer":
        return eval_caption(task, generator, models, sample, **kwargs)    
    elif task.cfg._name == "progrounding":
